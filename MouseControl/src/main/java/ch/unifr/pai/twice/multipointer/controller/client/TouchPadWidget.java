	...
	protected NativePreviewHandler keyboardPreviewHandler = new NativePreviewHandler() {
		...
		public void onPreviewNativeEvent(NativePreviewEvent event) {
			...
			case Event.ONKEYDOWN:
				...
				RemoteKeyDownEvent keyDownEvent = GWT.create(RemoteKeyDownEvent.class);
				keyDownEvent.keyCode = event.getNativeEvent().getKeyCode();
				CommunicationManager.getBidirectionalEventBus().fireEvent(keyDownEvent);
			...
			case Event.ONKEYUP: 
				...
				RemoteKeyUpEvent keyUpEvent = GWT.create(RemoteKeyUpEvent.class);
				keyUpEvent.keyCode = event.getNativeEvent().getKeyCode();
				CommunicationManager.getBidirectionalEventBus().fireEvent(keyUpEvent);
			...
			case Event.ONKEYPRESS:
				RemoteKeyPressEvent keyPressEvent = GWT.create(RemoteKeyPressEvent.class);
				keyPressEvent.keyCode = event.getNativeEvent().getKeyCode();
				keyPressEvent.charCode = event.getNativeEvent().getCharCode();

				CommunicationManager.getBidirectionalEventBus().fireEvent(keyPressEvent);
				break;
			}
		}
	};
		...
		if (color != null) {
			if (color.startsWith("#") && color.length() == 7) {
				try {
					...
					// 128 is the average brightness (16^2/2)
					return d > 128;
